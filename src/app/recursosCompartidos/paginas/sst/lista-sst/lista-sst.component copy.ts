...
import { Departamento } from 'src/app/recursosCompartidos/modelos/departamento.class';
import { DepartamentoInterface } from 'src/app/recursosCompartidos/modelos/departamento.interface';
import { SolicitudSTI } from 'src/app/recursosCompartidos/modelos/solicitudesst.interface';
...
import { GenerarPDFService } from 'src/app/recursosCompartidos/servicios/generar-pdf.service';
import { ServiciosService } from 'src/app/recursosCompartidos/servicios/servicios.service';
import { SstService } from 'src/app/recursosCompartidos/servicios/sst.service';
import { ValidarFechas } from 'src/app/recursosCompartidos/servicios/validarFechas';
import { SstComponent } from '../sst.component';

@Component({
  selector: 'app-lista-sst',
  templateUrl: './lista-sst.component.html',
  styleUrls: ['./lista-sst.component.scss']
})
export class ListaSstComponent implements OnInit {
  ...
  async ngOnInit():Promise<void>{
    ...
    if (logged=="false"){
      ...
      /*this.authServicio.departamentos.pipe(takeUntil(this.unSubscribe$))
                        .subscribe(datos => { this.deptos = datos; })
      ;*/
      ...
      if(this.deptoSesion.departamento=="JARDINERÍA Y TRANSPORTES"){
        ...
        /*this.servicios.allSST().pipe(takeUntil(this.unSubscribe$))
                      .subscribe(data => { this.arrSstI = data;
                                          this.totalRegistros = this.arrSstI.length;
                                          this.cambiarDepartamento_id_x_Departamento(this.arrSstI);
                      });*/
        ...
        this.dataSource.data = await this.cambiarDepartamento_id_x_Departamento(this.arrSstI, this.deptos);
        /**
        this.servicios.getColeccionSSTDepto(String(this.deptoSesion.id))
  ...
  async imprimir(element: any){
    if (this.deptoSesion.departamento == "JARDINERÍA Y TRANSPORTES"){
      ...
      //this.deptoInterface = await this.authServicio.getDepartamento_async(element.departamento_id, element);
      ...
      /*this.servicios.coleccion$('solicitudest',
            ref => ref.where('folio', '==', element.folio))
            .subscribe(response=>{
                this.ssti = response;
                console.log("consulta de datos: ", this.ssti);
                GenerarPDFService.generaPDF_ST(this.ssti,this.deptoSesion,Number(this.ssti.folio),this.ssti.id);
  ...
  async sstHoy(){
    ...
    /*
    this.servicios.coleccion$(
                        'solicitudesst',
                          ref => ref.where('salidaSt','==',this.hoy))
                  .pipe(takeUntil(this.unSubscribe$))
                  .subscribe(
                              response => { this.arrSstI = response;
                              //this.cambiarDepartamento_id_x_Departamento(this.arrSstI);
                      });
    */
  }
  ...
  async sstMostla(){
    ...
    this.servicios.coleccion$(
                        'solicitudesst',
                          ref => ref.where('salidaSt','==',this.mostla))
                  .pipe(takeUntil(this.unSubscribe$))
                  .subscribe(
                              response => { this.arrSstI = response;
                              //this.cambiarDepartamento_id_x_Departamento(this.arrSstI);
